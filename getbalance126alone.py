...
while row > i:
    ...
    if success:
        ...
        if not flag:
            ...
            chan.send(com)
            sleep(2)
            com='btfs daemon --enable-gc --chain-id 199\n'
            print(com)
            chan.send(com)
            sleep(4)
            flag=True
        ...
        while lis < maxnumber+1:
            try:
                ...
                if flag:
                    ...
                    result=bytes.decode(chan.recv(2048))
                    pat = '"BttcAddress": "([\s\S]*?)",'
                    id = re.search(pat, result).group(1)
                    print(id)
                    chan.send('btfs cheque bttbalance '+ str(id) +'\n')
                    sleep(5)
                    data=bytes.decode(chan.recv(2048))
                    if 'the balance is' in data:
                        pat='the balance is: (0|([1-9]\d*))(\.\d+)?'
                        ...
                        begin=begin+1
                        continue
                    BttWalletBalance = BttWalletBalance.replace('the balance is: ', '')

...
LastEditTime: 2022-03-05 13:33:53
LastEditors: honus
Description: 
FilePath: \test\delscreen_btt_2_1.py
'''
import os
import subprocess
...
while(True):
    ...
    if int(res[2])>1024:
        ...
        command='rm -rf screen_btt_2_1.py'
        os.system(command)
        command='rm -rf auto_btt_2_1.py'
        os.system(command)
        command='rm -rf wget-log'
        os.system(command)
        command='wget https://raw.githubusercontent.com/0honus0/test/main/auto_btt_2_1.py'
        subprocess.call(command,shell=True)
        sleep(60)
        flag = False
        num = 3
        while(not flag and num > 0):
            command = 'screen -dmS run python3 /root/auto_btt_2_1.py'
            subprocess.call(command,shell=True)
            print('rebooting...')
            command = 'screen -ls'
            res = os.popen(command).readlines()

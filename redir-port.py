...
while row > i:
    ...
    if success:
        ...
        stdin, stdout, stderr = client.exec_command(command)
        res=stdout.readlines()
        command="sudo rm -rf /etc/ss-tproxy/ss-tproxy.conf && wget -P /etc/ss-tproxy/ https://raw.githubusercontent.com/0honus0/test/main/ss-tproxy.conf.port"
        stdin, stdout, stderr = client.exec_command(command)
        res=stdout.readlines()
        command="mv /etc/ss-tproxy/ss-tproxy.conf.port /etc/ss-tproxy/ss-tproxy.conf"
        stdin, stdout, stderr = client.exec_command(command)
        res=stdout.readlines()
